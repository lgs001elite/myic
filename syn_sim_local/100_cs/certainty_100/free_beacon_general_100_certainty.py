...
def updatingCoorAssiDistime(icID, icSet, synScheme, scenario):
    ...
    if (synScheme == synSchemes.free_beacon):
        ...
        if bias != 0:
            bias = ic_distribution_cycle - bias
        icSet[icID].disTime = icSet[icID].disTime + bias +  dutyCycle + ic_distribution_cycle * uniform_integer_random(0, 1)
...
def statistics(logger, scenario, icNums, icSet1_1, icSet2_1, icSet3_1, icSet4_1, poweroffLabel):
    ...
    collisionFile_free_beacon = open(poweroffLabel +"free_beacon_collision_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
    # sets
