  ...
  while ( true ) {
    ...
    while ( g_if_inWork == true ) {
      ...
      if ( check_completeness( packet_receiver ) == true ) {
        int senderID = packet_receiver[ 5 ];
        int currLoc = packet_receiver[ 9 ];
        if ( g_curr_loc > 0 ) {
          g_delay_bias = IC_CYCLE - g_curr_loc;
        } else {
          g_delay_bias = 0;
        }
        producePackets( g_delay_bias, senderID );
