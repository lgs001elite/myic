...
time_threshold = 5000 # 10000, 100000, 1000000
working_cycle = 5  # 5, 10, 15,  30, 45
minCharge = 1
maxCharge = 100
...
def find(model):
    ...
    while (stopSignal != 0):
        ...
        if (waitingTimeS > waitingTimeR):
            ...
            findSet.append(waitingTimeS - lastSynTime)
            lastSynTime = waitingTimeS
            senderCharge = energy_model(model, 1)
            var1Index = var1Index + 1 
            delay = geoGenerator(returnParameter(senderCharge))
            waitingTimeS = senderCharge + 1 + delay + waitingTimeS
...
def greedy(model):
    ...
    while (stopSignal != 0):
        ...
        if (waitingTimeS > waitingTimeR):
            ...
            synCounter = synCounter + 1
            greedySet.append(waitingTimeS - lastSynTime)
            lastSynTime = waitingTimeS
            senderCharge = energy_model(model, 1)
            var1Index = var1Index + 1 
            waitingTimeS = senderCharge + 1 + waitingTimeS
...
def cycle(model):
    ...
    while (stopSignal != 0):
        ...
        if (waitingTimeS > waitingTimeR):
            ...
            synCounter = synCounter + 1
            cycleSet.append(waitingTimeS - lastSynTime)
            lastSynTime = waitingTimeS
            senderCharge = energy_model(model, 1)
            var1Index = var1Index + 1
            senderDelay = working_cycle - (senderCharge + 1) % working_cycle
...
def neutron_star(model):
    ...
    while (sumSender != sumReceiver):
        ...
        if (sumReceiver == sumSender):
            synCounter = synCounter + 1
            neutronStarSet.append(sumSender - lastSyntime)
            lastSyntime = sumSender
            tch_sender = energy_model(model, 1)
            delay_sender = working_cycle - ((tch_sender + 1) % working_cycle)
            var1Index = var1Index + 1
            sumSender = sumSender + delay_sender + tch_sender + 1
...
if __name__ == "__main__":
    for model in ["gauss", "expo", "uniform", "stairs"]:  # "gauss", "expo", "uniform", "cars", "stairs", "office", "washer", "jogging"
        var1Index = 0
        var2Index = 0
        greedySet = []
        neutronStarSet = []
