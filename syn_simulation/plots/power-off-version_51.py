...
def Free_beacon(distribution_cycle, indicator_cycle, scenario, nodeNums, icSet, pf_rate):
    ...
    print("Free_beacon finished"+str(i))
...
def Flync_Find(distribution_cycle, indicator_cycle, scenario, nodeNums, icSet, pf_rate):
    ...
    while i < repeation_number:
        ...
        for index in range(nodeNums):
            ...
            icSet[index].synTime = uniform_integer_random(0, tchTime)
            icSet[index].currentTraceIndex = uniform_integer_random(0, repeation_number)
    print("Flync finished")




def statistics(logger, scenario, pf_rate, icSet1_1, icSet2_1, icSet3_1, icSet4_1, poweroffLabel, icNums):
    global icSet1
    global icSet2
    global icSet3
    global icSet4
    ...
    for index in range(icNums):
        indexSet = 0
        for icNodes in [icSet1_1, icSet2_1, icSet3_1, icSet4_1]:
    ...
    for setIndex in range(3):
        if np.average(synListSets[3]) != 0:
            logger.info(synSchemeNames[setIndex] +"_averageEnhancedSyn: " + str(np.average(synListSets[setIndex]) / np.average(synListSets[3])))
        else:
            logger.info(synSchemeNames[setIndex] +"_averageEnhancedSyn: " + str(np.average(synListSets[setIndex])))
        if np.average(disListSets[3]) != 0:
            logger.info(synSchemeNames[setIndex] +"_averageEnhancedDis: " + str(np.average(disListSets[setIndex]) / np.average(disListSets[3])))
        else:
...
def sub_sub_task(scenario, fb_size, pf_rate, logger, icNums, poweroffLabel):
    global icSet1
    global icSet2
    global icSet3
    global icSet4
    ...
    futuresMoreMoreTasks.append(executorMoreMore.submit(Find, 0, 0, scenario, icNums, icSet1, pf_rate))
    futuresMoreMoreTasks.append(executorMoreMore.submit(Flync_Find, 0, 10, scenario, icNums, icSet2, pf_rate))
    futuresMoreMoreTasks.append(executorMoreMore.submit(Pulsar, pulsarDistributionCycle, 0, scenario, icNums, icSet3, pf_rate))
    futuresMoreMoreTasks.append(executorMoreMore.submit(Free_beacon, fb_size, min_co_prime(fb_size), scenario, icNums, icSet4, pf_rate))
    # Wait for all futures to complete
    for fTask in futuresMoreMoreTasks:
        fTask.result()
    statistics(logger, scenario, pf_rate, icSet1, icSet2, icSet3, icSet4, poweroffLabel, icNums)



def sub_task(fb_size, pf_rate, logger, icNums, poweroffLabel):
    # futuresMoreTasks = []
    # executorMore = ThreadPoolExecutor(max_workers=7)
    for scenario in range(7):
        sub_sub_task(scenario, fb_size, pf_rate, logger, icNums, poweroffLabel)
        #futuresMoreTasks.append(executorMore.submit(sub_sub_task, scenario, fb_size, pf_rate, logger, icNums, poweroffLabel))
    # for fTask in futuresMoreTasks:
    #     fTask.result()
...
if __name__ == "__main__":
    ...
    icNums = 30
    pulsarDistributionCycle = icNums + 1
    power_off_len = len(poweroff_rateset)
    poweroffLabel = "poweroff_"
    print(poweroffLabel)
    # define power_off tasks
    futuresTasks = []
    executor = ThreadPoolExecutor(max_workers=power_off_len)
    for i in range(power_off_len):
        # Create log
        logfilename = poweroffLabel+str(poweroff_rateset[i]) +"_results"
        ...
        #
        sub_task(Free_beacon_size, poweroff_rateset[i], logger, icNums, poweroffLabel)
        # futuresTasks.append(executor.submit(sub_task, Free_beacon_size, poweroff_rateset[i], logger, icNums, poweroffLabel))
