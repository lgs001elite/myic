...
# plot the data
fig, ax = plt.subplots(nrows=1, ncols=7, figsize=(21, 3),constrained_layout=True)
...
for i in range(1):
    for j in range(scenarios_len):
        ...
        for k in range(methods_len):
            boxplot = _ax.boxplot(data_sets[j][k][2], positions= [k + i * (methods_len + 1) + 1], patch_artist=True,
        ...
        # if (j == 3):
        #     _ax.set_title(node_sets_name[2]+" [nodes]")
        _ax.set_xticks([])
