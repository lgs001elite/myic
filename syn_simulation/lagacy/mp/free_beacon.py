...
def statistics(logger, scenario, icNums):
    global icSet1
    global icSet2
    global icSet3
    global icSet4
    ...
    # find
    disFile_find = open(poweroffLabel +"find_dis_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
    synFile_find = open(poweroffLabel +"find_syn_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
    collisionFile_find = open(poweroffLabel +"find_collision_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
    # flync find
    disFile_flync = open(poweroffLabel +"flync_dis_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
    synFile_flync = open(poweroffLabel +"flync_syn_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
    collisionFile_flync = open(poweroffLabel +"flync_collision_in_"+scenarios[scenario]
                                       +"_"+str(icNums)+".csv", 'w')
    # pulsar
    disFile_pulsar = open(poweroffLabel +"pulsar_dis_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
    synFile_pulsar = open(poweroffLabel +"pulsar_syn_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
    collisionFile_pulsar = open(poweroffLabel +"pulsar_collision_in_"+scenarios[scenario]
                                        +"_"+str(icNums)+".csv", 'w')
    # free_beacon
    disFile_free_beacon = open(poweroffLabel +"free_beacon_dis_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
    synFile_free_beacon = open(poweroffLabel +"free_beacon_syn_in_"+scenarios[scenario]+"_"+str(icNums)+".csv", 'w')
...
def sub_subTask(scenario, fb_size, icNums, logger):
     # starting to work
    icSet1 = []
    icSet2 = []
    icSet3 = []
    ...
    for fTask in futuresMoreMoreTasks:
        fTask.result()
    statistics(logger, scenario, icNums)


def subTask(fb_size, icNums, poweroffLabel):
    # Create log
    logfilename = poweroffLabel+str(icNums) +"_results.log"
    ...
    for scenario in  range(7):
        futuresMoreTasks.append(executorMore.submit(sub_subTask, scenario, fb_size, icNums, logger))
...
if __name__ == "__main__":
    ...
    poweroffLabel = "without_poweroff_"
     
    # for all concurrent jobs
    futuresTasks = []
    executor = ThreadPoolExecutor(max_workers=lengthNodeSet)
    for i in range(lengthNodeSet):
        futuresTasks.append(executor.submit(subTask,Free_beacon_size, general_nodeset[i], poweroffLabel))
