...
static bool g_receved_completed = false;
...
static void mesh_init( void ) {
  ...
  advertiser_instance_init( &m_discovery_advertiser, tx_complete_cb, m_adv_buffer_discovery, ADVERTISER_BUFFER_SIZE );
}
...
static void coordinator_role( ) {
  ...
  if ( g_synStrategy == FREEBEACON ) {
    g_current_loc_cycle = ( g_current_loc_cycle + 7 ) % WORKING_CYCLE_FREECON;
    g_current_loc_cycle = g_current_loc_cycle % WORKING_CYCLE_FREECON;
  } else {
    g_current_loc_cycle = 0;
  }
...
static void ble_execution( void ) {
  ...
  while ( true ) {
    ...
    ret = app_timer_start( g_my_sleep_timer_counter, APP_TIMER_TICKS( SLEEP_LEN ), NULL );
    APP_ERROR_CHECK( ret );
    ...
    g_receved_completed = false;
    producePackets( );
    coordinator_role( );
    g_if_trans = false;
    nrf_mesh_enable( );
    send2bearer( &m_discovery_advertiser, m_tx_buf_spi );
    while ( ( g_work_cycle_switch == true ) && ( g_if_trans == false ) ) {
      // send2bearer( &m_discovery_advertiser, m_tx_buf_spi );
    ...
    nrf_gpio_pin_clear( 28 ); // or nrf_gpio_pin_toggle
