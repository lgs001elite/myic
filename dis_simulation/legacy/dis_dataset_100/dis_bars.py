...
title ="100_distribution_cycles_"
...
for scenario in energy_scenarios:
    ...
    for method in syn_methods:
        ...
        for num in node_sets:
            file = open("no_poweroff_100_general_"+ method.lower()+"_dis_in_"+ scenario.upper()+"_"+ str(num)+".csv", mode='r')
...
# plot the data
fig, ax = plt.subplots(nrows=3, ncols=7, figsize=(10, 4),constrained_layout=True)
...
for i in range(nums_len):
    for j in range(scenarios_len):
        ...
        if j == 0:
           _ax.set_ylabel("Dis Time", fontsize=fontSize)
