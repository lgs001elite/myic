...
# plot the data
fig, ax = plt.subplots(nrows=5, ncols=7, figsize=(15, 15),constrained_layout=True)
...
for i in range(rates_len):
    for j in range(scenarios_len):
        ...
        for k in range(methods_len):
            _ax.boxplot(data_sets[i][k][j], positions= [k + i * (methods_len + 1) + 1],
            boxprops=dict(color=colors[k]),
