...
syn_methods_name = ["Pulsar", "FreeBeacon"]
...
title ="free_beacon_with_30_dis_"
...
for scenario in energy_scenarios:
    ...
    for method in syn_methods:
        ...
        for num in node_sets:
            file = open("no_poweroff_30_general_"+ method.lower()+"_dis_in_"+ scenario.upper()+"_"+ str(num)+".csv", mode='r')
...
# plot the data
fig, ax = plt.subplots(nrows=3, ncols=7, figsize=(10, 3),constrained_layout=True)
...
for i in range(nums_len):
    for j in range(scenarios_len):
        _ax = ax[i][j]
        for k in range(2):
            _ax.bar(syn_methods_name[k],data_sets[j][k][i], color=colors[k])
        if j == 0:
