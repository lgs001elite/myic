...
for scenario in energy_scenarios:
    ...
    for method in syn_methods:
        ...
        for num in node_sets:
            file = open("no_poweroff_100_general_"+ method.lower()+"_syn_in_"+ scenario.upper()+"_"+ str(num)+".csv", mode='r')
...
#             _ax.boxplot(data_sets[i][j][k], positions= [j + k * (methods_len + 1) + 1], patch_artist=True,
...
#                         flierprops=dict(marker='o', markerfacecolor=colors[j], markeredgecolor=colors[j], markersize=1, alpha=0.5))


#     xticks = [3 + (i * (methods_len+1)) for i in range(nums_len)]         
#     xtickslabels = node_sets 
#     _ax.set_xticks(xticks)
#     _ax.set_xticklabels(xtickslabels, fontsize=fontSize)            
#     _ax.set_title(energy_scenarios_names[i], fontsize=fontSize)
#     _ax.set_xlabel("# Battery-free Devices", fontsize=fontSize)
#     _ax.tick_params(axis='both', which='major', labelsize=fontSize)
...
#     for j in range(1, methods_len):
#         _ax.axvline(x=(j * (methods_len + 1)), color='gray', linestyle='dotted', linewidth=0.6)   
