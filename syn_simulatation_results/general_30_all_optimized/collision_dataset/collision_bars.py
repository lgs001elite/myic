...
title ="free_beacon_with_30_collision_"
...
for scenario in energy_scenarios:
    ...
    for method in syn_methods:
        ...
        for num in node_sets:
            file = open("no_poweroff_30_general_"+ method.lower()+"_collision_in_"+ scenario.upper()+"_"+ str(num)+".csv", mode='r')
...
# plot the data
fig, ax = plt.subplots(nrows=2, ncols=7, figsize=(21, 6),constrained_layout=True)
...
for i in range(2):
    for j in range(scenarios_len):
        ...
        for k in range(methods_len):
            _ax.bar(syn_methods_name[k],data_sets[j][k][i+1], color=colors[k])
        if j == 0:
           _ax.set_ylabel("Collision numbers", fontsize=fontSize)
        ...
        if (j == 3):
            _ax.set_title(node_sets_name[i+1]+" [nodes]")
        _ax.set_xticks([])
