...
fig, ax = plt.subplots(1, 2, figsize=(8, 4), sharey=True, gridspec_kw={'width_ratios': [3, 1]})
...
ax[0].set_ylabel('Charging time [ms]')
ax[0].set_xlabel('Charging cycle sequence')
...
ax_inset_left.set_ylim(0, 15000)  # Adjust the y-axis to start from 0 to 20000 for the zoomed section
...
ax_inset_box  = ax[1].inset_axes([0.1, 0.3, 0.2, 0.6])  # Place the inset outside the original violin plot
sns.boxplot(data=new_data, ax=ax_inset_box, width=0.3, showfliers=False, showmeans=True) 
...
plt.savefig("../../figures/trace.pdf", dpi=300)
