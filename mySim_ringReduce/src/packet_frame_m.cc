...
int packet_frameDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0]=='d' && strcmp(fieldName, "dest_id")==0) return base+0;
    if (fieldName[0]=='s' && strcmp(fieldName, "source_id")==0) return base+1;
...
int packet_frameDescriptor::getFieldArraySize(void *object, int field) const
{
    ...
    packet_frame *pp = (packet_frame *)object; (void)pp;
    switch (field) {
        default: return 0;
    }
}
...
std::string packet_frameDescriptor::getFieldValueAsString(void *object, int field, int i) const
{
    ...
    packet_frame *pp = (packet_frame *)object; (void)pp;
    switch (field) {
        case 0: return long2string(pp->getDest_id());
        case 1: return long2string(pp->getSource_id());
...
bool packet_frameDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    packet_frame *pp = (packet_frame *)object; (void)pp;
    switch (field) {
        case 0: pp->setDest_id(string2long(value)); return true;
        case 1: pp->setSource_id(string2long(value)); return true;
