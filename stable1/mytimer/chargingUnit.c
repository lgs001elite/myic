...
#define MAX_SYN_SET 1000
#define INITIAL_TIME 1099999
char RXData = 0x01;
char TXData = 0x01;
int chargeCycles = 0;
int slotUnitCounter = 0;
int lastReceivedData = -1;
char g_uartArr[2] = {0};
...
{
    uint16_t reUartData = combineBytes2(g_uartArr);
    ...
    {
        reUartData = reUartData - 0xafff - 1;
...
int main(void)
{
    ...
    slotUnitCounter  = (chargeCycles + 1) * SLOT_AMPLIFIER;
    //***************************************
    ...
    while (uartSwitch)
    {
        ...
        if ((slotUnitCounter == SLOT_AMPLIFIER) && (preventRepeatedUart == true))
        {
            P1OUT ^= BIT2;
            P6SEL0 ^= BIT0;
            P4OUT ^= BIT2;
...
#endif
{
    ...
    if ((slotUnitCounter > 0) && (g_sleepCounter == 0) && (slotStopCounter == false))
    {
        slotUnitCounter--;
    }
    if (g_sleepCounter > 0)
...
#endif
{
    switch(__even_in_range(UCA3IV, USCI_UART_UCTXCPTIFG))
    {
        ...
        case USCI_UART_UCRXIFG:
            if ((g_reCounter < 2) && (reEntrance == true))
            {
